...
import { Lightbulb, Send, ClipboardList, Check, X, Clock, Edit, Trash2, ChevronLeft, MessageSquare, Sparkles, Users } from "lucide-react";
...
const STATUS_ICON: Record<string, React.ReactNode> = { pending: <Clock className="w-3 h-3" />, approved: <Check className="w-3 h-3" />, rejected: <X className="w-3 h-3" /> };
...
export default function SuggestItemPage() {
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            ...
            if (res.success) toast.success("Suggestion deleted");
        ...
        <div className="min-h-screen bg-zayko-900 pb-24">
            ...
            <div className="bg-zayko-800/80 backdrop-blur-xl border-b border-white/[0.06] px-4 py-4 sm:px-6 sticky top-0 z-40">
                <div className="max-w-3xl mx-auto flex items-center gap-4">
                    ...
                    <div className="w-10 h-10 rounded-xl bg-amber-500/10 flex items-center justify-center shadow-lg shadow-amber-500/5"><Lightbulb className="w-5 h-5 text-amber-500" /></div>
                    <div>
            ...
            <div className="max-w-3xl mx-auto p-6 space-y-8">
                ...
                <div className="bg-zayko-800/50 border border-zayko-700 rounded-2xl p-6 animate-fade-in">
                    <h2 className="text-base font-display font-bold text-white mb-4 flex items-center gap-2">
                        <span className="w-8 h-8 bg-emerald-500/20 rounded-lg flex items-center justify-center text-sm"><Sparkles className="w-4 h-4 text-emerald-500" /></span>
                        Submit Suggestion
                    </h2>
                ...
                <div>
                    <h2 className="text-base font-display font-bold text-white mb-4 flex items-center gap-2">
                        <span className="w-8 h-8 bg-purple-500/20 rounded-lg flex items-center justify-center text-sm"><ClipboardList className="w-4 h-4 text-purple-400" /></span>
                        My Suggestions
                    </h2>
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="flex gap-2 pt-1">
                                                <button onClick={saveEdit} disabled={editSaving}
                                                    ...
                                                    {editSaving ? "Saving…" : <span className="flex items-center justify-center gap-2"><Check className="w-4 h-4" /> Save</span>}
                                                </button>
                                        ...
                                        <div className="flex items-center justify-between gap-3">
                                            <div className="min-w-0 flex-1">
                                                <div className="flex items-center gap-2 mb-1 flex-wrap">
                                                    ...
                                                    <span className={`flex items-center gap-1 text-xs px-2 py-0.5 rounded-full font-semibold ${STATUS_STYLE[s.status]}`}>
                                                        {STATUS_ICON[s.status]} <span>{s.status.charAt(0).toUpperCase() + s.status.slice(1)}</span>
                                                    </span>
                                                </div>
                                                {s.description && (
                                                ...
                                                <div className="flex items-center gap-3 mt-1.5 text-xs text-zayko-500">
                                                    <span className="flex items-center gap-1"><Users className="w-3 h-3" /> {s.totalRequests} request{s.totalRequests !== 1 ? "s" : ""}</span>
                                            ...
                                            {s.status === "pending" && (
                                                <div className="flex items-center gap-1.5 shrink-0">
                                                    <button onClick={() => startEdit(s)} className="p-2 rounded-xl text-sm bg-blue-500/10 text-blue-400 hover:bg-blue-500/20 transition-all flex justify-center items-center" title="Edit"><Edit className="w-4 h-4" /></button>
                                                    <button onClick={() => handleDelete(s.id)} className="p-2 rounded-xl text-sm bg-red-500/10 text-red-400 hover:bg-red-500/20 transition-all flex justify-center items-center" title="Delete"><Trash2 className="w-4 h-4" /></button>
                                                </div>
                                            )}
                                        </div>
                                    )}
